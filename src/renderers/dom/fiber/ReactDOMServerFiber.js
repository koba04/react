 ...
 *
 * @providesModule ReactDOMServerFiber
 * @flow
 */
...
findDOMNode._injectFiber(function(fiber: Fiber) {
  return DOMServerRenderer.findHostInstance(fiber);
});
...
var DOMServerRenderer = ReactFiberReconciler({
...
ReactGenericBatching.injection.injectFiberBatchedUpdates(DOMServerRenderer.batchedUpdates);
...
function renderSubtreeIntoContainer(parentComponent : ?ReactComponent<any, any, any>, element : ReactElement<any>, containerNode : DOMContainerElement | Document, callback: ?Function) {
  ...
  if (!container._reactRootContainer) {
    ...
    root = container._reactRootContainer = DOMServerRenderer.mountContainer(element, container, parentComponent, callback);
  } else {
    DOMServerRenderer.updateContainer(element, root = container._reactRootContainer, parentComponent, callback);
  }
  return DOMServerRenderer.getPublicRootInstance(root);
}
...
var ReactDOMServer = {
  ...
  unmountComponentAtNode(container : DOMContainerElement) {
    ...
    if (root) {
      ...
      DOMServerRenderer.unmountContainer(root);
    }
  },
  ...
  unstable_deferredUpdates: DOMServerRenderer.deferredUpdates,
